    ...
    useEffect(() => {
        ...
        const startConfetti = () => {
            ...
            for (let i in particles.current) {
                particles.current[i].tiltAngle += randomNumberGenerator(
                    0.01,
                    2,
                );
                particles.current[i].y +=
                    (Math.sin(0.4) + particles.current[i].area + 0.05) * 0.2;
                particles.current[i].tilt =
                    Math.cos(particles.current[i].tiltAngle) * 0.3;
                particles.current[i].draw();

                if (particles.current[i].y > height) {
                    particles.current[i] = new ParticleClass();
                    particles.current[i].color =
                        `rgb( ${randomNumberGenerator(0, 255)}, ${randomNumberGenerator(0, 255)}, ${randomNumberGenerator(0, 255)}`;
                }
            }
